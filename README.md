# ZombieOutbreak

## Windows Forms Project by: Tamara Stojanoska (231030), Davor Lazarov (233109) and Ognen Mladenovski (233108)

### Tutorial: https://www.youtube.com/watch?v=z2h_4_f6fD8

Македонски / English

### 1. Опис на апликацијата
Проектната задача претставува имплементација на игра со зомби, изработена со помош на C# и Windows Forms. Главната цел на играчот е да преживее колку што е можно подолго, избегнувајќи зомби суштества кои се појавуваат на екранот и се движат кон него. Играчот може да наиде на 

Начин на играње:
#### 1. Играта започнува со fullscreen режим.
#### 2. Првиот екран содржи копчиња: играј, излез, начин на играње. Има можност за избор на јазик (Македонски/Англиски) и музика додека не започне играта (песна: The Walking Dead Season 3 Comic Con Trailer Song Black - Kari Kimmel).
