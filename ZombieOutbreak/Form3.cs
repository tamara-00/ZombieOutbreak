...
namespace ZombieOutbreak
{
    public partial class Form3 : Form
    {
        ...
        public Form3(string language)
        {
            ...
            ammoRegenTimer.Interval = 10000; // 10 seconds
        ...
        private void Form3_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            switch (e.KeyCode)
            {
                case Keys.Left:
                    goLeft = true;
                    if (!IsZombieInDirection("left"))
                    {
                        facing = "left";
                        player.Image = Properties.Resources.rifle3;
                        player.Height = 180;
                        player.Width = 300;
                    }
                    break;
                ...
                case Keys.Right:
                    goRight = true;
                    if (!IsZombieInDirection("right"))
                    {
                        facing = "right";
                        player.Image = Properties.Resources.rifle1;
                        player.Height = 180;
                        player.Width = 300;
                    }
                    break;
                ...
                case Keys.Down:
                    goDown = true;
                    if (!IsZombieInDirection("down"))
                    {
                        facing = "down";
                        player.Image = Properties.Resources.rifle2;
                        player.Width = 180;
                        player.Height = 300;
                    }
                    break;
        ...
        private void RestartGame()
        {
            ...
            List<Control> powerUpsToRemove = new List<Control>();
            foreach (Control control in this.Controls)
            {
                if (control is PictureBox pic && (pic.Tag?.ToString().StartsWith("ammo") == true || pic.Tag?.ToString() == "health" || pic.Tag?.ToString() == "shield"))
                {
                    powerUpsToRemove.Add(control);
                }
            }
            foreach (Control control in powerUpsToRemove)
            {
